// One-time test: inserts docs/ROADMAP.md (plus its docs/roadmap/*.md phase
// children) as a `manual` + `section` tree under the "manuals" category for
// the real Codestash workspace — proving the doc-family-as-in-app-manual
// idea from ROADMAP.md's "Later: bringing this into the app itself" note.
// Run with: npx tsx --env-file=.env.local scripts/seed-roadmap-manual.ts
//
...
const sections: Node[] = [
  {
    ...
    blocks: [
      { type: "p", text: "Not the optimistic version." },
      {
        type: "list",
        items: [
          ...
          "**Production is live but broken** — `/` and every `/api/auth/*` call were 500ing as of the last check, root cause not yet found.",
    ...
    children: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: "p",
            text: "An unresolved production outage sits underneath this entire plan — Phase 0 exists because of it.",
          },
        ],
      },
      {
  ...
  },
  {
    ...
    children: [
      phase(
        "Phase 0 — Stabilize (you are here)",
        ...
        [
          ...
          "Identify the actual cause and fix it; confirm `/` and every `/api/auth/*` route returns 200",
          "Add `.github/workflows/ci.yml` running lint, typecheck, build",
          "Commit `.env.example`",
        ],
        "Full test suite and error monitoring — both explicitly Phase 6.",
        "Production serves every route without a 500; a broken build can't merge silently.",
      ),
      phase(
        "Phase 1 — Foundations",
...
async function main() {
  ...
  if (existing) {
    console.log(`Manual "${roadmapSlug}" already exists (id ${existing.id}) — nothing to do.`);
    return;
  }
  ...
  console.log(`Seeded manual "${roadmapSlug}" (${manualId}) with ${count} sections.`);
}
