// Keeps the in-app "Story" / "Setup" / "Roles & Billing Plan" manuals
// (first seeded by seed-doc-family-manuals.ts) in sync with STORY.md,
// SETUP.md, and ROLES-AND-BILLING-PLAN.md. Same relationship
// update-roadmap-manual.ts has to seed-roadmap-manual.ts: this one
// replaces each manual's section tree instead of no-op'ing when it
// already exists — re-run it whenever any of those three .md files change.
    ...
    sections: [
        ...
        blocks: [
          {
            ...
            items: [
              "Node.js + npm",
              "A Neon account (free tier is fine) — required from the start now; there's no static-catalog fallback to browse without a database.",
            ],
          },
        ],
      },
      ...
      },
      {
        ...
        blocks: [
          ...
          {
            type: "p",
            text: "Visit `/` to browse categories, or any manual directly, e.g. `/manuals/mastering-git`. **Requires the database setup below** — the catalog is 100% DB-backed with no static fallback; without `DATABASE_URL` set, `lib/db/index.ts` throws immediately.",
          },
        ],
      },
      {
            ...
            blocks: [
              {
                ...
                items: [
                  ...
                  "`CategoryConfig` (the 5 built-in categories' chrome — icon/Background per category) holds real function values — can't cross the Server→Client Component boundary as a prop. DB categories sidestep this: `category.icon` is a plain string resolved via `resolveIcon()`.",
      ...
      },
      {
        ...
        blocks: [
          {
            type: "p",
            text: "DB/auth: schema pushed to Neon; sign-up/sign-in, workspace creation, invites, and accepting an invite all work end-to-end. Platform superadmin status is still a manual, one-time, no-UI step.",
          },
          {
            type: "p",
          ...
          {
            type: "p",
            text: "Snippets no longer have their own table — retired in favor of storing a snippet as a `manual` row with one section and a single code block. The old physical `snippet` table (left over after the code stopped referencing it) has since been dropped from Postgres too.",
          },
          {
            type: "p",
            text: 'Categories are dynamic and reorderable for any workspace with categories seeded in the DB — but there\'s still no "create category" UI, so manual seeding is the only path today. The real workspace has all 6 of its categories (5 built-in + the custom `codestash` category holding these planning docs) seeded, 29 manual rows total.',
          },
          {
            type: "note",
...
async function main() {
  ...
  for (const doc of docs) {
    ...
    if (!existing) {
      console.log(`Manual "${doc.slug}" doesn't exist yet — run seed-doc-family-manuals.ts first.`);
      continue;
    }

    await db.delete(section).where(eq(section.manualId, existing.id));
    await db
      .update(manual)
      .set({ title: doc.title, subtitle: doc.subtitle })
      .where(eq(manual.id, existing.id));

    const count = await insertLevel(existing.id, doc.sections, null);
    console.log(`Refreshed manual "${doc.slug}" (${existing.id}) with ${count} sections.`);
  }
}
