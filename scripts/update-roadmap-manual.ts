...
const sections: Node[] = [
  {
    ...
    blocks: [
      { type: "p", text: "Not the optimistic version. Last updated 2026-09-03." },
      {
        type: "list",
        items: [
          "A real DB/auth layer — sign-up, sign-in, workspaces, invites, all working end-to-end. **The catalog itself is 100% DB-backed now, with zero static content left anywhere** (`lib/data/` holds only shared types) — fixed 2026-09-04, the home page was still rendering a hardcoded category list. Also reversed the same day: the catalog is no longer public — every route requires a real session (`proxy.ts` + a per-page server-verified check), a signed-out visitor is redirected to `/sign-in`.",
